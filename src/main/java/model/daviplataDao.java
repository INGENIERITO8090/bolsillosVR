...
public int registrar(daviplataVo dp) throws SQLException{
  ...
  public int  consultar_saldo_p(int id) throws SQLException{                 
        sql="select  saldo  from daviplata  where idUsuario="+id+";";  
        ...
        try {
            ...
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery(sql);
            System.out.println("sql consulta saldo ");
            while(rs.next()){
         //Escribir  en el setter cada valor encontrado
            dp.setSaldo(rs.getInt("Saldo"));  
            System.out.println("saldo dao "+dp.getSaldo());                   
     ...
     public int modificarsaldo(daviplataVo dp ) throws SQLException{   
        sql="update daviplata set saldo=(?) where idUsuario="+dp.getIdUsuario()+";"; 
        try {  
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
